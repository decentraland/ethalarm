...
export default class EventService {
      ...
      this.alarms
        ...
        .then(async results => {
          ...
          for (let x = 0; x < results.length; ++x) {
            ...
            const receipts = await this.alarms.getReceipt(
              alarm.id,
              tx_hash,
              event_name
            )
            if (!receipts.length) {
              if (!this.events[contract_address])
                this.events[contract_address] = []

              // If this event transaction already exists, simply update the block_height
              let event_to_add = this.events[
                contract_address
              ].filter(cur_event => {
                return (
                  cur_event.tx_hash === tx_hash &&
              ...
              if (
                !touched.some(touched_event => {
                  return touched_event === event_to_add
                })
              )
  ...
  getLastSyncBlock() {
    return this._alarm_sync_state_model
      .findAll({
        order: [['last_sync_block', 'DESC']],
        limit: 1
      })
      .then(results => {
  ...
  async watchEvents(web3, address, abi, event_names) {
    ...
    for (let x = 0; x < event_names.length; ++x) {
      if (!Object.keys(this.watching[address]).includes(event_names[x])) {
        ...
        this.watching[address][
          event_names[x]
        ] = my_event.watch((error, result) => {
          if (!error) {
            this.addSeen(result)
          }
        })
  ...
  async watchNewEvents(web3) {
    ...
    const to_watch = Object.keys(contracts).filter(address => {
      return (
        !this.watching[address] ||
        Object.keys(this.watching[address]).filter(event_name => {
  ...
  async getAlarmsToTrigger(currentBlockHeight) {
    ...
    for (let address in this.events) {
      for (let x = 0; x < this.events[address].length; ++x) {
        ...
        const alarms = (await this.alarms.getAlarms({
          addresses: [address]
        })).filter(alarm => {
        ...
        for (let y = 0; y < alarms.length; ++y) {
          ...
          const blocks_passed =
            event.block_height <=
            currentBlockHeight - (alarm.block_confirmations - 1)
          if (!blocks_passed) continue

          const receipt = await this.alarms.getReceipt(
            alarm.id,
            event.tx_hash,
            event.event_name
          )
